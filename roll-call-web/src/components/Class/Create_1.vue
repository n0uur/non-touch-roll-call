  ...
  methods: {
    nextPage() {
      Swal.fire({
        ...
        if (result.value) {
          if (
            this.classForm.instructor != "" &&
            this.classForm.subject != "" &&
            this.classForm.password != ""
          ) {
            axios({
              method: "POST",
              ...
              .then(res => {
                if (res.data.status == 200)
                  this.$router.push({
                    path: "/class/view/" + res.data.classid
                  });
                else {
                  Swal.fire("ข้อผิดพลาด", "พบข้อผิดพลาด", "error");
                }
              })
              .catch(err => {
                console.log(err);
